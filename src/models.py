...
class M_REPORT_TGF(Base):
    ...
    folder_link: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Линк на отчет')
    folder_short: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Путь короткий')
    folder_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Папка отчета')
    rgf: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='РГФ')
    tgf_hmao: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ ХМАО')
    tgf_ynao: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ ЯНАО')
    tgf_kras: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ КРАС')
    tgf_ekat: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ ЕКАТЕРИНБУРГ')
    tgf_omsk: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ ОМСК')
    tgf_novo: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ НОВОСИБИРСК')
    tgf_tomsk: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ ТОМСК')
    tgf_more: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ МОРЕ')
    tgf_tmn: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ ТЮМЕНЬ')
    tgf_kurgan: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ КУРГАН')
    tgf: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='ТГФ')
    report_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Имя отчета')
    author_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Автор отчета')
    year_str: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='Год отчета')
    year_int: Mapped[int] = mapped_column(Integer, index=True, nullable=True, comment='Год отчета (число)')
    territory_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Територия')

    subrf_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Субьект РФ')
    list_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='№ листа карты')
    part_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='№ партии')
    areaoil: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Площадь')
    field: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Месторождение')
    lu: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Лицензионный участок')
    pi_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Полезное ископаемое')
    fin_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Источник финансирования')
    org_name: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Организация')
    zsniigg_report: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='Отчет в ЗапСибНИИГГ')
    inf_report: Mapped[str] = mapped_column(String(length=255), index=True, nullable=True, comment='Инф. отчет')
    vid_rab: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Вид работ')

    comments: Mapped[str] = mapped_column(TEXT, index=True, nullable=True, comment='Комментарий')
    lat: Mapped[float] = mapped_column(Float, nullable=True, comment='Широта')  # ormar.Float(precision=21, scale=18)
    lon: Mapped[float] = mapped_column(Float, nullable=True, comment='Долгота')  # ormar.Float(precision=21, scale=18)
    is_alive: Mapped[bool] = mapped_column(Boolean, nullable=True, comment='Признак актуальности')  #
    report_fts: Mapped[str] = mapped_column(TSVECTOR, nullable=True, comment='Полнотекстовый индекс')
