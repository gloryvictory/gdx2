...
async def report_excel_file_read(file_in: str):
    ...
    try:
        ...
        for value in worksheet.iter_rows(min_row=2, max_row=worksheet.max_row, min_col=1, max_col=30, values_only=True):
            print(f"{cnt}")
            cnt = cnt + 1
            folder_root = ''
            folder_link = ''
            ...
            if value[11]:
                ...
                # print(tgf_kurgan)
            if value[12]:
                rgf = str_tgf_format(str(value[12]))
                print(rgf)
            #
            # if len(tgf_tmn):
            #     tgf = 'ТюмТГФ'
