...
from src.api.well.services import well_reload, well_get_all, well_get_all_count, well_get_geojson_file, \
    well_get_by_area, well_get_area_count
...
async def get_well_get_all_count():
...
@well_router.get(path="/{area}",
                 status_code=200,
                 name='Получить список скважин по конкретной площади',
                 tags=['Скважины'],
                 description='Получает список Скважин и координаты  по конкретной площади'
                 )
async def get_well_get_by_area(area: str):
