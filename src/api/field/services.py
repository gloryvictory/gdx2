...
from src.models import M_NSI_FIELD


async def fields_reload():
    content = {"msg": "Success"}
    file_geojson = os.path.join(cfg.FOLDER_BASE, cfg.FOLDER_DATA, cfg.FIELDS_FILE_GEOJSON_IN)
    print(os.getcwd())
    print(file_geojson)
    file_geojson_out = os.path.join(cfg.FOLDER_BASE, cfg.FOLDER_GEOJSON_OUT, cfg.FIELDS_FILE_GEOJSON_OUT)
    name_field = cfg.FIELDS_NAME_FIELD  # 'name_ru'
    crs_out = cfg.CRS_OUT
    ...
    try:
        ...
        log = set_logger(cfg.FIELDS_FILE_LOG)

        log.info(gdf1)

        async with async_session_maker() as session:
            # await M_NSI_NGP.objects.delete(each=True)
            stmt = text(f"TRUNCATE {M_NSI_FIELD.__tablename__} RESTART IDENTITY;")
            ...
            for i in range(0, len(gdf1)):
                ...
                stmt = insert(M_NSI_FIELD).values(name_ru=_name_ru, lon=_lon, lat=_lat, crs=_crs, hash=_hash)
                await session.execute(stmt)
                await session.commit()
            count = await session.scalar(select(func.count(M_NSI_FIELD.id)))
...
async def fields_get_all_count():
    content = {"msg": f"error"}
    try:
        async with async_session_maker() as session:
            res = await session.scalar(select(func.count(M_NSI_FIELD.id)))
            content = {"msg": "Success", "count": res}
            return content
    except Exception as e:
        cont_err = f"fail. can't read ext from table ({M_NSI_FIELD.__tablename__})"
        content = {"msg": "error", "data": f"Exception occurred {str(e)} . {cont_err}"}
        print(content)
    finally:
        if session is not None:
